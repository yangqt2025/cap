...
(
    ...
    sum double comment '总分',
    final_score int comment '最终得分',
    suggestion text comment '改进建议',
    overall_suggestion text comment '整体建议',
    analysis_strengths text comment '分析优势',
    analysis_improvements text comment '分析改进',
    suggestions text comment '具体建议',
    question_type varchar(50) comment '题目类型（面试/申论）',
    create_time datetime default CURRENT_TIMESTAMP not null comment '创建时间',
    update_time datetime default CURRENT_TIMESTAMP not null on update CURRENT_TIMESTAMP comment '更新时间',
    is_delete tinyint default 0 not null comment '是否删除'
)
